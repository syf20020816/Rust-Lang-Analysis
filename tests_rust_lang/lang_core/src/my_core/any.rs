...
/// 大多数类型实现Any。然而，任何包含非静态引用的类型都不包含
...
impl TypeId {
    ...
    ///
    /// 实际上是使用编译器内部函数intrinsics::type_id::<T>()获取对应类型的ID
    ///
    ...
    /// 当我们使用这个函数则必须要定 ： #![feature(const_type_name)]
    ///
    /// 因为实际上stable的实现是没有const的
    ///
    /// > stable : `pub const fn type_name<T: ?Sized>() -> &'static str`
    pub const fn type_name<T: ?Sized>() -> &'static str {
        intrinsics::type_name::<T>()
    }

    // pub fn is_string<T: ?Sized + Any>(_s: &T) -> TypeId {
    //     TypeId::of::<T>()
    // }

    /// 获取值的类型
    /// 通过使用上方的type_name
    pub fn type_name_of_val<T: ?Sized>(_s: &T) -> &'static str {
        Self::type_name::<T>()
    }
}